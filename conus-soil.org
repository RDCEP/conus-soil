...
* Collate the cell IDs, weights, and multiple variables 
...
#+BEGIN_SRC R
  ...
  names( fractionStack)[ 1:3] <- c( "pixel", "cell", "cropFreq")
  
  fractionDt <- data.table( fractionStack[], key= "pixel,cell")
  ## setkey( fractionDt, cell)
  fractionDt <- fractionDt[ conusCells[ I( hasConus)]]
  fractionDt[, hasConus := NULL]
  fractionDt[, total := sand + silt + clay]
  fractionDt <- fractionDt[ !is.na( total)][ total > 0]
  scaledFractionDt <- copy( fractionDt)
  setkey( scaledFractionDt, pixel, cell)
  scaledFractionDt <- scaledFractionDt[, lapply(
    .SD, function( x) x*100/total), by= "pixel,cell"]
  scaledFractionDt[, `:=`( cropFreq= NULL, total= NULL)]
  
  fractionDt <- fractionDt[, list( cell, cropFreq), key= cell][ scaledFractionDt]
  
  fractionDt <- fractionDt[ scaledFractionDt]
  
  sumCropFreq <- fractionDt[, list( crop= sum( cropFreq)), by= cell]
  ## sumCropFreq <- sumCropFreq
  
  fractionCropDt <- fractionDt[, lapply(
    .SD, weighted.mean, w= cropFreq, na.rm= TRUE), by= cell]
  
  fractionCropDt <- fractionCropDt[, cropFreq := NULL]
  ...
  fractionAllDt <-  fractionDt[ total == 100, lapply(
    .SD, mean, na.rm= TRUE), by= cell]
  fractionAllDt[, cropFreq := NULL]
  fractionAllDt[, total := sand + silt + clay]
  ...
  fractionDt <- `[`(
    x= fractionDt,
    i= is.nan( fractionCrop),
    fractionCrop := 0)
  
  fractionAll <- raster( world)
  fractionAll[ fractionDt$cell] <- fractionDt$fractionAll
  fractionCrop <- raster( world)
  fractionCrop[ fractionDt$cell] <- fractionDt$fractionCrop
